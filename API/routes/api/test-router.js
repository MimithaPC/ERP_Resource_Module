...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        const result = await client.query('SELECT * from cerpschema.test');
...
router.get("/:aid", async (req, res) => {
    ...
    try {
        ...
        const result = await client.query(
            'SELECT * from cerpschema.test where test_id  = $1', 
            [req.params.aid]
        );
        console.log("test:", result.rows[0]);
        res.json(result.rows);
...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        const result = await client.query(
            'INSERT INTO cerpschema.test (test_id , test_name) VALUES ($1, $2) RETURNING *',
            [test_id , test_name]
        );
