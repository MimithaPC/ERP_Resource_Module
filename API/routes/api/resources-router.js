...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        const result = await client.query('SELECT * from resources.items');
...
router.get("/:aid", async (req, res) => {
    ...
    try {
        ...
        const result = await client.query(
            'SELECT * from resources.items where item_id = $1', 
            [req.params.aid]
        );
        console.log("resources:", result.rows[0]);
        res.json(result.rows);
...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        const result = await client.query(
            'INSERT INTO resources.items (item_name, item_type, unit_id, description, created_by, updated_by, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *',
...
router.put("/:id", async (req, res) => {
    ...
    try {
        ...
        const result = await client.query(
            `UPDATE resources.items SET item_name = $1, item_type = $2, unit_id = $3, description = $4, updated_by = $5, updated_at = $6 WHERE item_id = $7 RETURNING *`,
...
router.delete("/:id", async (req, res) => {
    ...
    try {
        ...
        const { id } = req.params;
        const result = await client.query(
            'DELETE FROM resources.items WHERE item_id = $1 RETURNING *',
            [id]
        );
        if (result.rows.length === 0) {
            return res.status(404).json({ message: "Resource not found" });
